...
{
    public class GeneroRepository : IBancoRepository<Genero>
    {
        ...
        {
            Genero data = await _context.Genero.FirstOrDefaultAsync(x => x.id == id);
            return data;
        }
        public async Task<Respuesta> Add(Genero model)
        {
        ...
        public async Task<Respuesta> Update(int id, Genero model)
        {
        ...
        {
            _resp = await ExecuteQuery(3, id, new Genero());
            return _resp;
        }

        private async Task<Respuesta> ExecuteQuery(int accion,int id, Genero model)
        {
            string query = "EXEC dbo.spMantenimientoGenero @movimiento, @id, @descripcion, @RetVal, @ErrorMessage";
